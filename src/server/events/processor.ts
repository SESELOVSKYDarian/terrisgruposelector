...
export type InternalEvent = { id: string; event_type: string; payload: { recipientId: string; slotId?: string; slotDate?: string; detail?: string; territoryRoundId?: string; title?: string; targetUrl?: string } };
...
export async function processInternalNotification(repository: InternalEventRepository, event: InternalEvent) {
  ...
  if (event.event_type === "VISIT_REPORT_DUE" && payload.territoryRoundId && payload.title) {
    const notification: InternalNotification = { eventId: event.id, recipientId: payload.recipientId, type: event.event_type, title: payload.title, description: "Recordatorio: completá el informe de la salida.", entityType: "territory_round", entityId: payload.territoryRoundId, targetUrl: payload.targetUrl ?? "/?view=outings" };
