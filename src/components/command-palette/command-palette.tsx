...
import { CalendarDays, ChevronRight, ClipboardList, Command, CornerDownLeft, Hash, MapPin, Megaphone, Search, Settings, UserRound, Users } from "lucide-react";
...
export function CommandPalette({ access, onCreateOuting, onNavigate, open, onOpenChange, userId }: { access: ShellAccess; onCreateOuting?: () => void; onNavigate: (view: string) => void; open: boolean; onOpenChange: (open: boolean) => void; userId: string }) {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    const commands = [command("command:dashboard", "Ir al resumen", "Inicio", "dashboard", Command, "outing"), ...(access.canManageTerritories ? [command("command:territories", "Ir a Territorios", "Operación", "territories", Hash, "territory")] : []), ...(access.canUseReservations ? [command("command:reservations", "Ir a Reservas", "Operación", "reservations", CalendarDays, "outing")] : []), ...(access.isConductor ? [command("command:my-outings", "Ir a Mis salidas", "Operación", "myOutings", ClipboardList, "outing")] : []), ...(access.hasPersonalTerritory ? [command("command:personal-territory", "Ir a Mi territorio", "Operación", "personalTerritory", MapPin, "territory")] : []), ...(access.canManageUsers ? [command("command:users", "Ir a Usuarios", "Administración", "users", Users, "user")] : []), ...(access.canPlanOutings ? [command("command:outings", "Ir a Salidas", "Operación", "outings", CalendarDays, "outing")] : []), ...[command("command:announcements", "Ir a Anuncios", "Inicio", "announcements", Megaphone, "announcement")], ...(access.canPublishAnnouncements ? [command("command:new-announcement", "Nuevo anuncio", "Publicar un aviso para todos", "announcements", Megaphone, "announcement")] : []), ...(access.canManageSystem ? [command("command:settings", "Ir a Ajustes", "Sistema", "settings", Settings, "outing")] : []), ...(onCreateOuting ? [command("command:new-outing", "Nueva salida", "Crear la próxima planificación semanal", "outings", CalendarDays, "outing", onCreateOuting)] : [])];
  ...
  }, [access.canManageSystem, access.canManageTerritories, access.canManageUsers, access.canPlanOutings, access.canPublishAnnouncements, access.canUseReservations, access.hasPersonalTerritory, access.isConductor, entries, onCreateOuting, onNavigate, onOpenChange, query, recents, userId]);
