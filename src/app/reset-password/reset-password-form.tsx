...
export function ResetPasswordForm({ token }: { token: string }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error((await response.json().catch(() => null))?.error ?? "No se pudo restablecer la contraseña.");
      setDone(true);
    } catch (cause) {
  ...
  return (
    <AuthShell>
      <section className="glass-panel floating-card w-full rounded-[1.75rem] p-6 sm:p-7">
        ...
        <h1 className="mt-5 text-2xl font-semibold tracking-tight text-foreground">Elegir nueva contraseña</h1>

        {!token ? (
          <p className="mt-4 text-sm text-danger">El link no es valido. Pedi uno nuevo desde la pantalla de ingreso.</p>
        ) : done ? (
          <>
            <p className="mt-4 text-sm leading-6 text-muted">Tu contraseña se actualizo. Ya podes volver a ingresar.</p>
