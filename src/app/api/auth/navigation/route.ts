...
export async function GET() {
  ...
  const personal = await createAdminSupabaseClient().from("personal_territory_assignments").select("id", { count: "exact", head: true }).eq("profile_id", profile.id).eq("status", "ACTIVE");
  return ok({ access: { ...navigationAccess(permissions), canPlanOutings: planning.canPlan || planning.canPublish, canManageTerritories: territories.canManage, canViewS13: territories.canViewS13, hasPersonalTerritory: !personal.error && (personal.count ?? 0) > 0 } });
}
