...
export function ForcePasswordChangeScreen({ onDone, onLogout }: { onDone: () => Promise<void>; onLogout: () => void }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error((await response.json().catch(() => null))?.error ?? "No se pudo cambiar la contrasena.");
      await onDone();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "No se pudo cambiar la contrasena.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="relative z-10 grid min-h-screen place-items-center px-4 py-8">
      <section className="glass-panel floating-card w-full max-w-md rounded-[1.75rem] p-6 sm:p-7">
        ...
        <h1 className="mt-5 text-2xl font-semibold tracking-tight text-foreground">Elegi una contrasena nueva</h1>
        <p className="mt-2 text-sm leading-6 text-muted">Tu contrasena actual es temporal. Elegi una nueva para seguir.</p>
